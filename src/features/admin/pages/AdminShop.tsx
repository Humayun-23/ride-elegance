...
import { getAdminShops, updateAdminShop, uploadAdminShopImage, deleteAdminShop } from "@/features/admin/services/adminService";
...
export default function AdminShop() {
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      const updated = (await updateAdminShop(selected.id, selected)).data;
      let updatedWithImage = updated;
      if (shopImage) {
        try {
          const formData = new FormData();
          formData.append("file", shopImage);
          updatedWithImage = (await uploadAdminShopImage(updated.id, formData, {
  ...
  const remove = async () => {
    ...
    try {
      await deleteAdminShop(selected.id);
